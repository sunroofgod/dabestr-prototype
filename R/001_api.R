...
load <- function(
  ...
  ## Added validity check to ensure that each variable in idx is present in the x column
  unique_x <- unique(data[[name_x]])
  for (i in 1:length(unlist_idx)) {
    if (isFALSE(unlist_idx[i] %in% unique_x)) {
      cli::cli_abort(c("{.field idx} contains variable not present in {.field x}.", 
                       "x" = "Ensure that idx does not have any variables not present in dataset."))
    }
  }
