...
create_dfs_for_sankey <-  function(float_contrast = FALSE,
  ...
  bar_width <- ifelse(float_contrast, 0.15, 0.3)
  scale_factor_sig <- ifelse(float_contrast, 0.7, 0.425)
  ...
  for (group in idx) {
    ...
    for (i in 1: (group_length - 1)) {
      ...
      sf_start1 <- ss #- gap / 2  #success_success$SS[1]
      sf_start2 <- means_c_t[ind] - gap/2 # / 2 - gap / 4
      sf_end1 <- means_c_t[ind + 1] + gap/2 # / 2 + gap / 4
      sf_end2 <- 1 - ff #+ gap / 2 #failure_failire$FF[1]
      
      
      # find values for upper flppied flow success to failure flow
      fs_start1 <- 1 - ff
      fs_start2 <- means_c_t[ind] + gap/2  #/ 2 + gap / 4
      fs_end1 <- means_c_t[ind + 1] - gap/2 #/ 2 - gap / 4
      fs_end2 <- ss
    ...
    group_being_processed <- group_being_processed + 1
    x_start <- x_start + 1
    ind <- ind + 1
  }
...
plot_raw <- function(dabest_effectsize_obj, float_contrast, plot_kwargs) {
  ...
  bar_width <- ifelse(float_contrast, 0.15, 0.04)
  ...
  if(isFALSE(float_contrast)) {
    ...
    if(isTRUE(is_deltadelta)) {
      raw_plot <- cowplot::plot_grid(
        plotlist = list(raw_plot + theme(legend.position="none"), NULL), 
        nrow = 1, 
        ncol = 2, 
        rel_widths = c(0.9, 0.1)
        )
...
```{r}
pdf(file = "Images/2group_float_true.pdf",
    width = 6,
    height = 8,
    pointsize = 12)
dabest_plot(dabest_obj.mean_diff, TRUE)
...
pdf(file = "Images/2group_float_false.pdf",
    width = 8,
    height = 6,
    pointsize = 12)
...
```{r}
png(file = "Images/2group_float_true.png",
    width = 5.5,
    height = 5.5,
    units = "in",
    pointsize = 12,
...
jpeg(file = "Images/2group_float_false.jpeg",
    width = 5.5,
    height = 5.5,
    units = "in",
    pointsize = 12,
...
## testing export as pdf/png/jepg
```{r}
pdf(file = "Images/multigroup_unpaired.pdf",
    width = 8,
    height = 6,
    pointsize = 12)
...
```{r}
png(file = "Images/multigroup_unpaired.png",
    width = 6,
    height = 6,
    units = "in",
    pointsize = 12,
...
jpeg(file = "Images/multigroup_unpaired.jpeg",
    width = 6,
    height = 6,
    units = "in",
    pointsize = 12,
...
## unpaired
```{r}
dabest_obj_multigroup_deltadelta <- load(data = my.data, x = Genotype, y = Measurement, delta2 = TRUE, experiment =Treatment, idx = list(c("W Placebo","M Placebo"),c("W Drug","M Drug")))
