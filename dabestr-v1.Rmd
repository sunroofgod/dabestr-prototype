...
load <- function(
  ...
  if (isTRUE(delta2)) {
    ...
    data <- data %>%
      ...
      unite(!!enquo_x,c(!!enquo_x,!!enquo_experiment),sep = " ",remove=FALSE)
    enquo_colour <- data$grouping
    is_colour <- TRUE
  }
  ...
  raw_plot <- switch(
    ...
    "sankey" =
      ...
      geom_sankeyflow(data = flow1, 
                      ...
                      aes(x = x, y = y, fill = "#db6159")) +
      geom_sankeyflow(data = flow2, 
                      aes(x = x, y = y, fill = "#818181")) +
...
my.data <- wide.data %>%
  tidyr::gather(key = Treatment, value = Measurement, -ID, -Genotype, -Dummy)
```
...
## paired
```{r}
dabest_obj_multigroup_deltadelta_baseline <- load(data = my.data, x = Genotype, y = Measurement, delta2 = TRUE,experiment =Treatment,idx = list(c("W Placebo","M Placebo"),c("W Drug","M Drug")), paired = "baseline", id = ID, colour = NA)
