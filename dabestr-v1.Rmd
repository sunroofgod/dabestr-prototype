...
calculate_group_variance <- function(ctrl_var, 
                                     ...
                                     test_var, 
                                     test_N) {
  ctrl_var/ctrl_N + test_var/test_N
}
...
boot_weighted_row <- function(boot_result, ci){
  ...
  weighted_boostrap <- Reduce("+", weighted_result)
  weighted_difference <- calculate_weighted_delta(weight = boot_result$weight,
                                                  differences = boot_result$difference)
  bca_weighted <- bca(theta =weighted_boostrap)
  pct_interval <- confinterval(weighted_boostrap, ci/100)
  boot_last_row <- list(
    control_group = 'Minimeta Overall Test',
    test_group = 'Minimeta Overall Test',
    bootstraps = list(as.vector(weighted_boostrap)),
    nboots = length(weighted_boostrap),
...
calculate_weighted_delta <- function(weight, differences) {
  denom <- sum(weight)
  num <- sapply(1:length(weight), function(i) weight[i] * differences[i])
  num <- Reduce("+", num)
  return(num / denom)
}
...
```{r}
dabest_obj_multigroup_minimeta <- load(data = my.data, x = Group, y = Measurement, minimeta = TRUE, idx = list(c("Control1", "Group1", "Group2")))
...
```{r}
dabest_obj_multigroup_minimeta.mean_diff$boot_result
```