...
```{r}
two_groups_unpaired <- load(df, x = Group, y = Measurement,
...
```{r, eval = FALSE}
two_groups_unpaired_ci90 = load(df, x = Group, y = Measurement,
...
After loading in the data via `load()`, these `effect_size()` functions are then used.
...
### Statistical tests
Since v0.3.0, dabestr will report the p-value of the non-parametric two-sided approximate permutation t-test. This is also known as the Monte Carlo permutation test.
...
```{r, eval = FALSE}
two_groups_unpaired.hedges_g <- hedges_g(two_groups_unpaired)
print(two_groups_unpaired.hedges_g$permtest_pvals$pvalues)
```
...
```{r}
multi_2group <- load(df, x = Group, y = Measurement,
                     idx = list(c("Control 1", "Test 1"), 
                                c("Control 2", "Test 2"))
                     )
multi_2group %>% mean_diff() %>% dabest_plot()
```
...
```{r}
share_control <- load(df, x = Group, y = Measurement,
                      idx = c("Control 1", "Test 1", "Test 2", "Test 3",
                              "Test 4", "Test 5", "Test 6")
                      )
# print(shared_control)
shared_control.mean_diff <- mean_diff(share_control)
...
```{r}
multi_groups <- load(df, x = Group, y = Measurement,
                     idx = list(c("Control 1", "Test 1"), 
