...
plot_delta <- function(dabest_effectsize_obj, float_contrast) {
  ...
  for (i in row_num) {
    ...
    # to change: need to change to be applied for multi-plot --> likely extrapolate into a different 
    # function that handles the creation of the df
    df_for_violin <- data.frame(x = c(NA, x_coords_ci),
                                y = c(NA, y_coords_ci),
                                tag = c("tag1", rep("tag2", 512))) %>%
      arrange(tag, x, y)
