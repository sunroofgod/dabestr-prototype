...
mean_diff <- function(dabest_obj) {
  ...
  if(is_paired){
    result <- bootstrap(dabest_obj, effect_size_func, boot_labs = "Paired\nmean difference")
    print_dabest(result,"mean_diff")
    return(result)
  }
  result <- bootstrap(dabest_obj, effect_size_func, boot_labs = "Mean difference")
  print_dabest(result,"mean_diff")
  return(result)
}
...
median_diff <- function(dabest_obj) {
  ...
  if(is_paired){
    result <- bootstrap(dabest_obj, effect_size_func, boot_labs = "Paired\nmedian difference")
    print_dabest(result,median_diff)
    return(result)
  }
  result <- bootstrap(dabest_obj, effect_size_func, boot_labs = "Median difference")
  print_dabest(result,"median_diff")
  return(result)
}
...
cohens_d <- function(dabest_obj) {
  ...
  result <- bootstrap(dabest_obj, effect_size_func, boot_labs = "Cohen's d")
  print_dabest(result,"cohens_d")
  return(result)
}
...
hedges_g <- function(dabest_obj) {
  ...
  result <- bootstrap(dabest_obj, effect_size_func, boot_labs = "Hedges' g")
  print_dabest(result,"hedges_g")
  return(result)
}
...
cliffs_delta <- function(dabest_obj) {
  ...
  result <- bootstrap(dabest_obj, effect_size_func, boot_labs = "Cliffs' delta")
  print_dabest(result,"cliffs_delta")
  return(result)
}
...
cohens_h <- function(dabest_obj){
  ...
  result <- bootstrap(dabest_obj, effect_size_func, boot_labs = "Cohen's h")
  print_dabest(result,"cohens_h")
  return(result)
}
...
create_dfs_for_nonflow_tufte_lines <- function(idx,
  ...
  for (i in (1: total_length)){
    ...
    row <- tufte_lines_df %>% 
      filter(!!enquo_x == group_name)
    new_tufte_lines_df <- bind_rows(new_tufte_lines_df, row)
  }
...
=======
    if(isTRUE(is_deltadelta)) {
      ...
      extra_yaxis_plot <- cowplot::plot_grid(NULL, extra_yaxis_plot, NULL, ncol = 1, rel_heights = c(0.50, 0.402, 0.098))
...
>>>>>>> 47ca352ad45b932408f25d516793ff9aa601e03f
    if(isTRUE(is_colour)) {
...
dabest_plot(dabest_obj_multigroup.mean_diff, FALSE, custom_palette = "jama", zero_dot = FALSE)
```
...
png(file = "figures/multigroup/multigroup_unpaired.png",
    ...
    res = 1200,
    bg = "white")
dabest_plot(dabest_obj_multigroup.mean_diff, FALSE, es_marker_size = 1, raw_marker_spread = 1.5, raw_marker_size = 1.2)
dev.off()
