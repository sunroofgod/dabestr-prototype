...
load <- function(
  ...
  ##Check that id_col is not NULL if is_paired is TRUE(Not working)
  if (isTRUE(is_paired) & quo_is_null(enquo_id_col)) {
    stop("`paired` is TRUE but no `id_col` was supplied.")
  }
  
  ## Check that paired must be either "baseline" or "sequential"
  if (isTRUE(is_paired)) {
    paired_check <- arg_match(paired,c("baseline","sequential"),error_arg = "paired")
  }
  
  ## Check for valid mini-meta 
  if (isTRUE(minimeta)) {
    if (isTRUE(proportional)) {
      stop("`proportional` and `minimeta` cannot be TRUE at the same time.")
    } else if (isTRUE(delta2)) {
      stop("`delta2` and `minimeta` cannot be TRUE at the same time.")
    }
    
    all_idx_lengths <- sapply(idx,length)
    if (any(all_idx_lengths!=2)==TRUE) {
      stop("`minimeta` is TRUE, but some idx does not consist of exactly 2 groups.")
    }
  }
...
bootstrap <- function(
  ...
  enquo_effect_size_func <- enquo(effect_size_func)
  
  quoname_effect_size_func <- as_name(enquo_effect_size_func)
  ...
  if (isTRUE(is_paired) && boot_labs == "Cliffs' delta") {
    stop("`paired` is not NULL; therefore Cliff's delta is not defined.")
  } else if (isTRUE(proportional) && !(boot_labs %in% c("Mean difference","Cohen's h"))) {
    stop("`proportional` is TRUE; therefore effect size other than mean_diff and cohens_h is not defined.")
  }
