...
PermutationTest <- function(control,
  ...
  for (i in 1:permutation_count) {
    ...
    control_var <- var(control_sample, na.rm = TRUE)
    test_var <- var(test_sample, na.rm = TRUE)
...
pvals_statistics <- function(control, 
  ...
  if (!is.null(is_paired) && !proportional) {
    ...
    wilcoxon <- wilcox.test(control, test)
    ...
    if (effect_size != "median_diff") {
      ...
      paired_t <- t.test(control, test, paired = TRUE, na.rm = TRUE)
    ...
    table <- matrix(c(sum(control == 0 & test == 0), sum(control == 0 & test == 1),
                    ...
                    nrow = 2, byrow = TRUE)
    mcnemar_result <- mcnemar.test(table, correct = TRUE)
    ...
    # Brunner-Munzel test
    brunner_munzel <- brunnermunzel.test(control, test, na.rm = TRUE)
    ...
    kruskal <- kruskal.test(list(control, test))
    ...
    tryCatch({
      mann_whitney <- wilcox.test(control, test, alternative = "two.sided")
...
# Function to calculate pval and perms
```{r}
...
dabest_plot(dabest_obj_multigroup.mean_diff, FALSE, custom_palette = "jama", zero_dot = TRUE, raw_marker_side_shift = 0.2, raw_marker_spread = 2, asymmetric_side = "left", show_baseline_ec = TRUE)
```
