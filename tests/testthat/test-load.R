testthat::test_that("Able to load dataset", {
  ...
  expect_no_error(dabestr::load(dataset, x = Group, y = Measurement, idx = c("Control1", "Test1")))
})

testthat::test_that("Able to detect non-valid params", {
  dataset <- generate_non_proportional_dataset()
  expect_error(dabestr::load(dataset, x = Grou, y = Measurement, 
                             idx = c("Control1", "Test1")),
               regexp = "Column x is not in data")
  expect_error(dabestr::load(dataset, x = Group, y = Measuremen, 
                             idx = c("Control1", "Test1")),
               regexp = "Column y is not in data")
  expect_error(dabestr::load(dataset, x = Group, y = Measurement, 
                             idx = c("Control1", "Test1"), id_col = I),
               regexp = "Column id_col is not in data")
  expect_error(dabestr::load(dataset, x = Group, y = Measurement, 
                             idx = c("Control1", "Test1"), colour = Grou),
               regexp = "Column colour is not in data")
  expect_error(dabestr::load(dataset, x = Group, y = Measurement, 
                             idx = c("Control1")),
               regexp = "does not consist of at least 2 groups")
  expect_error(dabestr::load(dataset, x = Group, y = Measurement, 
                             idx = list(c("Control1", "Test1"), c("Control2"))),
               regexp = "does not consist of at least 2 groups")
})
...
testthat::test_that("Able to detect non-valid params for proportional = TRUE", {
  dataset <- generate_non_proportional_dataset()
  expect_error(dabestr::load(dataset, x = Group, y = Measurement, 
                             idx = c("Control1", "Test1"), proportional = TRUE),
...
testthat::test_that("Able to detect non-valid params for is_paired = TRUE", {
  dataset <- generate_non_proportional_dataset()
  expect_error(dabestr::load(dataset, x = Group, y = Measurement, 
                             idx = c("Control1", "Test1", "Test2"), paired = "baseline"),
               regexp = "is TRUE but no id_col was supplied")
  expect_error(dabestr::load(dataset, x = Group, y = Measurement, 
                             idx = c("Control1", "Test1", "Test2"), paired = "some",
                             id_col = ID),
               regexp = "is not 'baseline' or 'sequential'.")
})

testthat::test_that("Able to detect non-valid params for minimeta = TRUE", {
  dataset <- generate_proportional_dataset()
  expect_error(dabestr::load(dataset, x = Group, y = Success, 
                             idx = c("Control1", "Test1"), id_col = ID, 
...
testthat::test_that("Able to detect non-valid params for delta2 = TRUE", {
  ...
  dataset <- generate_deltadelta_dataset()
  expect_error(dabestr::load(dataset, x = Genotype, y = Measurement, 
                             delta2 = TRUE, experiment = Treatment, 
                             idx = list(c("W Placebo","M Placebo", "W Drug")), 
                             colour = Genotype),
               regexp = "experiment does not have a length of 2.")
})