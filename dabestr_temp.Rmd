...
df_for_tufte <- function(raw_data, 
                         enquo_x, 
                         enquo_y, 
                         proportional, 
                         is_paired = FALSE){
...
#Sigmoid Functions designed to plot geom_sankeyflow
```{r}
...
plot_raw <- function(dabest_effectsize_obj, float_contrast) {
  ...
  plot_groupsizes <-  unlist(lapply(idx, length))
  float.contrast <- float_contrast
  # added scale factor for gem sankey flow to fit x-axis symmetrically
  scale_factor_sig <- 0.8
  gap <- 0.05
  
  effsize_type <- dabest_effectsize_obj$delta_y_labels
  
  # list all columns or groups being compared
  all_groups <- unlist(idx)
  
  # calculate group sizes for percentage calculations
  N <- Ns$n[1]
  
  ID <- dabest_effectsize_obj$enquo_id_col
  
  # adapted from the original r version
  ...
  if (raw_x_max == 2 &&
    ...
    for (subplot_groups in idx) {
      subplot  <- dplyr::filter(raw_data, !!x_enquo %in% subplot_groups)
      for_plot[[length(for.plot) + 1]] <- subplot
    }

    for_plot <- dplyr::bind_rows(for_plot)
  }
  ...
  if (isTRUE(is_paired)) {
    if(isTRUE(proportional)){
      # index to extract the start values for plots
      ind <- 1
...
# from ss to mean plus gap
      sig1 <- sigmoid(ind + 0.1, scale_factor_sig, value_start1, value_end1)
      sig2 <- sigmoid(ind + 0.1, scale_factor_sig, value_start2, value_end2)
      sig1 <- arrange(sig1, desc(x))
      sig3 <- flipped_sig(ind + 0.1, scale_factor_sig, flow_start1, flow_end1)
      sig4 <- flipped_sig(ind + 0.1 , scale_factor_sig, flow_start2, flow_end2)
      ...
      raw_plot <- raw_plot +
        ...
        geom_sankeybar(data = data_for_bars, 
                 aes(x = x_axis_raw,
                     ysuccess = y_success, 
                     yfailure = y_failure, 
                     proportionsuccess = proportion_success, width = bar_width)) +
    ...
    # Adding tufte lines
      ...
      raw_plot <- ggplot(raw_data, 
                         aes(x = x_axis_raw, y = !!enquo_y, 
                             colour =      !!enquo_colour, 
                             group = !!enquo_id_col)) +
  ...
  if (isTRUE(float.contrast)){
    ...
    raw_plot <- raw_plot + 
      ...
      geom_segment(linewidth = 0.8, 
                   ...
                   y = raw_y_min, 
                   yend = raw_y_min)
    # if it is paired but not proportional data just draw the full summary line
    if (isFALSE(proportional)){
      raw_plot <- raw_plot +
        # Draw summary lines
        geom_segment(colour = "black",
                 linewidth = 0.3,
                 aes(x = 1, xend = 3, y = control_summary, 
...
# paired proportional
```{r}
dabest_obj_proportional.cohens_h.paired <- load(data = my.data.proportional, 
                                  x = Group, 
                                  y = Success, 
                                  idx = c("Control1", "Test1"),
                                  proportional = TRUE, is_paired = TRUE) %>%
  cohens_h()
```
# paired proportional plots
```{r}

dabest_plot(dabest_obj_proportional.cohens_h.paired, TRUE)
dabest_plot(dabest_obj_proportional.cohens_h.paired, FALSE)
```
